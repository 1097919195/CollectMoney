...
public class SlideDeleteWith extends ViewGroup{
    ...
    class MyDrawHelper extends ViewDragHelper.Callback {
        ...
        public int clampViewPositionHorizontal(View child, int left, int dx) {
            ...
            if(mDelete == child || mMinus ==child || mPlus==child){ // 解决删除部分左右拖动的越界问题
...
//            else{ // touch的是mDelete
//                int tempContentLeft = left - mContentWidth - mMinusWidth - mPlusWidth;
...
//            }
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            ...
            // 这里没必要分来两个孩子判断
            if(-mConLeft>mDeleteWidth/2+mPlusWidth/2+mMinusWidth/2){  // mDelete展示起来
                isShowDelete(true);
    ...
    public void isShowDelete(boolean isShowDelete){
        if(isShowDelete){
            ...
            viewDragHelper.smoothSlideViewTo(mDelete,mContentWidth-mDeleteWidth-mMinusWidth - mPlusWidth,0);
            viewDragHelper.smoothSlideViewTo(mMinus,mContentWidth - mMinusWidth - mPlusWidth,0);
            viewDragHelper.smoothSlideViewTo(mPlus,mContentWidth-mPlusWidth,0);
        }else{
